...
int PWM =0;
 ...
 float initial_MPU=0;
 float ang_HMC=0;
 float initial_HMC=350;//angle reference when MPU is left and HMC is at right
 float new_time=0;
 float prev_time=0;
 int count=0;
 float time_change=0;
...
void turn_zero_radius()
{
    ...
    if (pid_error< (-0.5))   //right
    {
      digitalWrite(IN1, HIGH);
      digitalWrite(IN2, LOW);
      digitalWrite(IN3, LOW);
      digitalWrite(IN4, HIGH);
      if(abs(pid_error) > abs(2*target_angle))
      {
        PWM =abs( 2*target_angle);
      }
      PWM = abs(pid_error);
      PWM = (double)PWM * 255/abs(2*target_angle);
      //PWM = map(PWM,0,2*target_angle,0,255);
      Serial.print("turn right with PWM=");
      Serial.print(PWM);
      Serial.println();
      analogWrite(EN1,PWM );
      analogWrite(EN2,PWM );
    }
    else if (pid_error > 0.5)  //left
    {
     ...
     if(abs(pid_error) >abs( 2*target_angle))
      ...
      PWM = (double)PWM * 255/abs(2*target_angle);
      //PWM = map(PWM,0,2*target_angle,0,255);
      Serial.print("turn left with PWM=");
      Serial.print(PWM);
      Serial.println();
      analogWrite(EN1, PWM);
      analogWrite(EN2, PWM);
